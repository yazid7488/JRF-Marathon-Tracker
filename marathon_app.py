...
# Cleaned up CSS for spacing and readability
st.markdown("""
    <style>
    .main { background-color: #0e1117; color: #ffffff; }
    h1 { font-size: 1.3rem !important; margin-bottom: 0px !important; }
    
    /* Keep the weeks tight together */
...
# Dashboard Row
st.markdown("""
    <div style="display: flex; justify-content: space-between; gap: 8px; padding-bottom: 20px; padding-top: 10px;">
...
for idx, row in df.iterrows():
    phase_val = str(row.get('Phase', 'BUILD')).upper()
    phase_emoji = emoji_map.get(phase_val, "🏃")
    p_color = row.get('PhaseColor', '#2ECC71')
    
    header_text = f"{row['Week']} | {row['Date']} | {row['Total Vol']} | {phase_val} {phase_emoji}"
    
    # Simple CSS targeting for the left accent color
    st.markdown(f"""
        <style>
