...
st.markdown("""
    ...
    /* Prevent content from clashing with header when expanded */
    [data-testid="stExpanderDetails"] {
        padding: 1.2rem 1rem 1rem 1rem !important;
    }

    /* Run Type Colors (Left bar on inner cards) */
...
# Weekly View wrapped in a single container to fix spacing
with st.container():
    for idx, row in df.iterrows():
        p_color = row.get('PhaseColor', '#2ECC71')
        header_text = f"{row['Week']} | {row['Date']} | {row['Total Vol']} | {row['Phase']}"
        
        # Applying style based on the Phase text to ensure colors stick
        st.markdown(f"""
            <style>
            div[data-testid="stExpander"]:nth-of-type({idx+1}) {{
                border-left: 8px solid {p_color} !important;
